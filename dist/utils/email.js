...
const resend = new resend_1.Resend(environment_1.env.RESEND_API_KEY);
// Your app's frontend URL (for links in emails)
const APP_URL = environment_1.env.FRONTEND_URL || 'http://localhost:5173';
const FROM_EMAIL = environment_1.env.FROM_EMAIL || 'FinWallet <onboarding@resend.dev>';
/**
...
async function sendPasswordResetEmail(email, token) {
    ...
    try {
      ...
      `,
        ...
        logger_1.logger.info(`Password reset email sent to ${email}`, { emailId: data?.id });
        return data;
    }
    catch (err) {
        logger_1.logger.error('Email service error:', err);
        throw err;
    }
}
/**
...
async function sendVerificationEmail(email, token) {
    ...
    try {
      ...
      `,
        ...
        logger_1.logger.info(`Verification email sent to ${email}`, { emailId: data?.id });
        return data;
    }
    catch (err) {
        logger_1.logger.error('Email service error:', err);
...
async function sendWelcomeEmail(email, firstName) {
    try {
      ...
      `,
        ...
        logger_1.logger.info(`Welcome email sent to ${email}`, { emailId: data?.id });
        return data;
    }
    catch (err) {
        logger_1.logger.error('Email service error:', err);
 ...
 */
async function sendGenericEmail(to, // Recipient email address
subject, // Email subject line
message // Email content/body (can include HTML)
) {
    try {
        const { data, error } = await resend.emails.send({
            from: FROM_EMAIL,
      ...
      `,
        ...
        if (error) {
            logger_1.logger.error('Failed to send generic email:', error);
            throw new Error('Failed to send email');
        }
        logger_1.logger.info(`Generic email sent to ${to}`, { emailId: data?.id, subject });
        return data;
    }
    catch (err) {
        logger_1.logger.error('Email service error:', err);
